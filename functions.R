...
computeProbsGES <- function(planTab,p) {
  ...
  for(j in seq_len(ncol(combs))) {
    ...
    for(i1 in 1:nrow(g1)) {
      ...
      totIndLT <- totIndLT + sum(ifelse(g1[i1,'value']<g2[,'value'],probii,0))
    }
    pG <- rbind(pG,data.frame(Deal1=d1,
                              Deal2=d2,
                              p1gt2=totIndGT/totProb,
                              p1lt2=totIndLT/totProb))
  }
  pG
}
...
plotPosterior <- function(p,wishPrice=c(),size=2) {
  ...
  plot <- ggplot() +
    ...
    xlab('Value per action ratio (dollars/action)') +
...
assembleResultTable = function(actions, successes, values, wishPrice) {
  ...
  ptab$pAboveTargetValue = pbeta(wishPrice/values,
